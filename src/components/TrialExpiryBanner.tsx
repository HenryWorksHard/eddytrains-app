...
export default function TrialExpiryBanner({ daysRemaining }: TrialExpiryBannerProps) {
  ...
  if (isExpired) {
    ...
    subtitle = isNativeApp 
      ? 'Visit app.cmpdcollective.com to subscribe and restore access.'
      : 'Subscribe now to restore access to all your programs and client data.'
    ...
    subtitle = isNativeApp
      ? 'Visit app.cmpdcollective.com to choose a plan.'
  ...
  return (
    <div className={`${bgColor} border rounded-xl p-4 flex items-center justify-between`}>
      ...
      {/* Hide upgrade button in native app for App Store compliance */}
      {!isNativeApp && (
        <Link 
          href="/billing" 
