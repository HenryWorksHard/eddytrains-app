...
export async function GET(request: NextRequest) {
  try {
    ...
    // Swap-aware lookup: include logs where swapped_exercise_name
    // matches the requested name, PLUS logs in any slot originally
    // named like the requested exercise where no swap happened.
    ...
    // Cascade fix (2026-08-26): match on the save-time snapshot before the
    // slot-id set, so pre-edit history (exercise_id nulled by a program
    // edit) still counts — otherwise the progression chart went empty
    // after any edit while the summary card still showed the PR.
