...
export function SlideOutMenu({ isOpen, onClose }: SlideOutMenuProps) {
  ...
  const handleNavClick = (href: string) => {
    // Navigate first - menu will close when route changes
    router.push(href)
  }
          ...
          {menuItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all active:scale-95 active:bg-zinc-700 ${
                  isActive 
          ...
          {bottomItems.map((item) => {
            ...
            return (
              <button
                key={item.href}
                onClick={() => handleNavClick(item.href)}
                className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-zinc-400 hover:bg-zinc-800 hover:text-white transition-all active:scale-95 active:bg-zinc-700"
              >
                <Icon className="w-5 h-5" />
                <span className="font-medium">{item.label}</span>
              </button>
